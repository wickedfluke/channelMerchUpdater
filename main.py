...
async def callback_handler(event):
    ...
    if data == "add_admin":
        ...
        await event.edit("Invia il nome utente (es. @username) da aggiungere come admin.")
    
    elif data == "select_channels":
        user_states[event.sender_id] = 'waiting_for_channel_link'
        await event.edit("Invia l'username (es. @username) o il link del canale da aggiungere.")

    elif data == "add_product":
        user_states[event.sender_id] = 'waiting_for_product_name'
        await event.edit("Invia il nome del prodotto da aggiungere.")
        ...
        if products:  
            ...
            await event.edit("Seleziona il prodotto:", buttons=buttons)
        ...
        await event.edit("Seleziona lo stato:",
            buttons=[
        ...
        await event.edit("Invia il nome del prodotto da eliminare.")

    elif data == "remove_channel":
        user_states[event.sender_id] = 'waiting_for_channel_removal'
        await event.edit("Invia l'ID del canale che desideri rimuovere.")

    elif data == "remove_admin":
        user_states[event.sender_id] = 'waiting_for_admin_removal'
        await event.edit("Invia l'username dell'admin che vuoi rimuovere.")

    elif data == "set_times":
        user_states[event.sender_id] = 'waiting_for_times'
        await event.edit("Invia due orari nel formato HH:MM (es. 10:00 18:00).")
    ...
    if event.sender_id in user_states:
        ...
        if state == 'waiting_for_username':
            ...
            if len(times) == 2:
                ...
                await event.respond(f"Formato orario non valido. Invia due orari nel formato HH:MM.",
                                    buttons=[Button.inline("Home", b"home")]
                )
            user_states.pop(event.sender_id)
            ...
            try:
                ...
                if user.id in admins:
                    ...
                    await event.respond(f"Questo utente non è un admin.",
                                        buttons=[Button.inline("Home", b"home")]
                    )
            except Exception as e:
                await event.respond(f"Errore: {e}",
